...
import { Heart, MessageSquare, AtSign, UserPlus, Bell, CheckCheck, MessageCircle, Phone, Eye } from 'lucide-react';
...
export default function NotificationsPage() {
  ...
  const [notifications, setNotifications] = useState<Notification[]>(liveNotifications || []);
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      ...
      const { data, error } = await (supabase as any).rpc('get_notifications', {
        p_user_id: user.id,
        p_limit: 50,
        p_offset: 0
      });
