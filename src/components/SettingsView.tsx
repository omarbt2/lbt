...
export default function SettingsView({
  ...
  const [activeSubpage, setActiveSubpage] = useState<'personal' | 'security' | 'notifications' | 'faq' | 'about' | 'pro' | null>(null);
  ...
  const [activeDevices, setActiveDevices] = useState([
    { id: 1, type: 'phone', name: 'iPhone 15 Pro', loc: 'New York, USA', time: 'Active now' },
    { id: 2, type: 'pc', name: 'MacBook Pro (Safari)', loc: 'Algiers, Algeria', time: '2h ago' },
    ...
    try {
      ...
      if (user) {
        await supabase.from('profiles').update({
          display_name: editName, username: editUsername.toLowerCase(),
          avatar_url: editPic, bio: editBio,
          phone: `${phonePrefix} ${phoneNum}`, dob: dob || null, gender: gender || null,
        } as any).eq('id', user.id);
      }
    } catch (err) {
      setSaveFeedback('Failed to save profile.'); return;
    }
    onUpdateUser({ ...currentUser, name: editName, username: editUsername, avatar: editPic, bio: editBio });
      ...
      <div className="flex flex-col gap-4 animate-fadeIn py-3 max-w-xl mx-auto text-on-surface">
            ...
            <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 overflow-hidden">
              <div className="p-4 border-b border-outline-variant/10 flex justify-between items-center">
                ...
                {activeDevices.length > 1 && (
                    <button onClick={() => setActiveDevices(prev => prev.filter(d => d.id === 1))}
                    className="text-[10px] font-bold text-error" aria-label="Log out of all devices">Log Out All</button>
                )}
              </div>
              {activeDevices.map(dev => (
                <div key={dev.id} className="flex items-center justify-between p-4 border-b border-outline-variant/5 last:border-0">
                  <div className="flex items-center gap-3">
                    {dev.type === 'phone' ? <Smartphone className="w-4 h-4 text-primary" /> : <Laptop className="w-4 h-4 text-primary" />}
                    <div>
                      <span className="text-xs font-bold text-on-surface block">{dev.name}</span>
                      <span className="text-[10px] text-outline">{dev.loc} · {dev.time}</span>
                    </div>
                  </div>
                  {dev.id !== 1 && (
                    <button onClick={() => setActiveDevices(prev => prev.filter(x => x.id !== dev.id))}
                      className="text-[10px] font-bold text-error">Revoke</button>
                  )}
                </div>
              ))}
            </div>
        ...
        {activeSubpage === 'notifications' && (
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 overflow-hidden animate-fadeIn">
            {[
              { label: 'Likes', icon: <Heart className="w-4 h-4 text-error" />, state: notifLikes, toggle: () => setNotifLikes(!notifLikes) },
              { label: 'Comments', icon: <MessageSquare className="w-4 h-4 text-primary" />, state: notifComments, toggle: () => setNotifComments(!notifComments) },
              { label: 'Direct Messages', icon: <Mail className="w-4 h-4 text-on-surface" />, state: notifDMs, toggle: () => setNotifDMs(!notifDMs) },
              { label: 'New Followers', icon: <Star className="w-4 h-4 text-on-surface-variant" />, state: notifFollowers, toggle: () => setNotifFollowers(!notifFollowers) },
            ].map((item, i) => (
  ...
  return (
    <div className="flex flex-col gap-5 animate-fadeIn py-3 max-w-xl mx-auto text-on-surface" id="settings_master">
      ...
      {/* Group: Preferences */}
      <section>
        <h4 className="text-[10px] font-extrabold text-primary mb-2 px-1 uppercase tracking-widest">Preferences</h4>
        <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 overflow-hidden divide-y divide-outline-variant/10">
          <div onClick={() => setActiveSubpage('notifications')}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-xs font-bold text-on-surface block">Notifications</span>
          ...
          <div className="flex items-center justify-between p-4">
            ...
            <button onClick={onThemeToggle}
              className={`w-10 h-5.5 rounded-full transition-all relative p-0.5 ${darkMode ? 'bg-primary' : 'bg-outline-variant'}`}
              aria-label={darkMode ? 'Switch to light theme' : 'Switch to dark theme'}
            >
              <div className={`w-4.5 h-4.5 bg-white rounded-full shadow transition-all ${darkMode ? 'translate-x-[18px]' : ''}`} />
            </button>
          </div>
        </div>
      </section>
